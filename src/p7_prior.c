...
*/
  ...
  pri->ti = esl_mixdchlet_Create(1, 2);  // insert transitions; single component; 2 params
  pri->td = esl_mixdchlet_Create(1, 2);  // delete transitions; single component; 2 params
  pri->em = esl_mixdchlet_Create(num_comp, 23); // match emissions; X component; 23 params
  pri->ei = esl_mixdchlet_Create(1, 23); // insert emissions; single component; 23 params
  ...
  for (q = 0; q < num_comp; q++)
    {
      pri->em->pq[q] = defmq[q];
      esl_vec_DCopy(defm[q], 23, pri->em->alpha[q]);
    }
  ...
  /* I don't understand why these wouldn't match the overall frequencies */
  //esl_vec_DCopy(defm[q], 23, pri->ei->alpha[0]);

  for (q = 0; q < 23; q++)
    {
      pri->ei->alpha[0][q] = defm[0][q]*1000000;
    }

  //  esl_vec_DSet(pri->ei->alpha[0], 23, 1.0);
